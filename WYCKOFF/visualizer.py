...
class KLineVisualizer:
    ...
    def batch_plot_results(self, results: List, data_manager, 
        ...
        for result in results[:max_plots]:
            try:
                symbol = result.get('代码', result.symbol if hasattr(result, 'symbol') else '')
                name = result.get('名称', result.name if hasattr(result, 'name') else '')
            
                df = data_manager.get_daily_data(symbol)  # 移除 days 参数
                if df.empty or len(df) < 20:
                    continue
